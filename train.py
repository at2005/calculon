...
#         mask = torch.cat([torch.ones(dim), torch.zeros(dim).cuda(local_rank)], dim=0)
#         self.mask = mask.view(2, -1).transpose(1,0).flatten()[: dim]
#         # self.register_buffer("mask", mask)
#         thetas = torch.arange(0, dim // 2).cuda(local_rank)
#         thetas = -2 * (thetas - 1) / dim
#         thetas = torch.exp(np.log(10000) * thetas)
#         self.thetas_repeated = thetas.repeat_interleave(2) * torch.arange(0, seq_len).view(-1, 1).cuda(local_rank)  # seq_len, model_width
        
#     def forward(self, x : torch.Tensor):
#         B, T, _ = x.shape
#         cos_terms = self.thetas_repeated.cos() * x
#         swapped = x.view(-1, 2).flip(1).reshape(B, T, -1)
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q,k,v = self.to_qkv(x).chunk(3, dim=-1) # shape is now (batch_size, seq_len, dim_head * heads) for q,k,v
...
class MoE(nn.Module):
    def __init__(self, dim, num_experts):
        ...
        self.expert_weights_1 = nn.Parameter(torch.empty(self.num_experts, dim, dim))
        self.expert_weights_2 = nn.Parameter(torch.empty(self.num_experts, dim, dim))
        self.expert_weights_3 = nn.Parameter(torch.empty(self.num_experts, dim, dim))
        ...
        self.expert_biases_3 = nn.Parameter(torch.zeros(self.num_experts, dim, 1))

        nn.init.xavier_normal_(self.expert_weights_1)
        nn.init.xavier_normal_(self.expert_weights_2)
        nn.init.xavier_normal_(self.expert_weights_3)
...
class TransformerBlock(nn.Module):
    def __init__(self, dim):
        ...
        self.moe = MoE(dim, num_experts=8)
...
class Transformer(nn.Module):
    def __init__(self, dim, num_layers=6):
        ...
        self.pos_range = torch.arange(seq_len).cuda(local_rank)
...
def main():
    model = Transformer(dim).cuda(local_rank)
    ...
    for epoch in tqdm(range(num_steps), disable=False, file=sys.stdout):
        ...
        if torch.distributed.get_rank() == 0:
            if epoch % 100 == 0:
                save_checkpoint(ddp_model, optimizer, epoch, step, loss, f"checkpoint_{epoch}_{step}.pt")
